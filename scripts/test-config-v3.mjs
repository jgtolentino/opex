 ...
 *
 * Usage:
 *   node scripts/test-config-v3.mjs
 */

import { loadConfig, validateConfig, formatValidationReport } from '../packages/core/dist/index.js'
...
async function main() {
  ...
  try {
    ...
    console.log('📋 Loading configuration from opex.config.test.ts...')
    const config = await loadConfig('./opex.config.test.ts')
    ...
    if (error.zodError) {
      ...
      console.error(error.zodError.errors)
    }
